...
def execute():
    ...
    # Check if preprocessed data should be reused
    if config.REUSE_PREPROCESSED_DATA:
        # Check if the preprocessed data file exists
        assert os.path.exists(os.path.join(config.DATA_DIR, "processed", f"{config.TEXT_SECTION_TYPE}_{'_'.join(config.TRAIN_DATA_INPUT_TYPES)}_processed.csv")), 'Preprocessed data not found! Please set REUSE_PREPROCESSED_DATA to False in config.py and run the pipeline again.'
    ...
    if not config.REUSE_PREGENERATED_EMBEDDINGS:
        ...
        # Check if pregenerated embeddings file exists
