...
# Try importing the 'config' and 'utils' modules. If it fails, import them from the current directory.
try:
...
# Define a function to sample random sentences from a given text
def sample_sentences(text: str, n: int = 1) -> list:
    """
    Sample N sentences from text
    """
...
# Define a function to sample a validation set from the test set
...
# Define a function to build GPT prompts from a dataset
def build_gpt_prompt(data: list, prompt_type=None, save=False) -> str:
    """
    Build a GPT prompt from a dataset
    """
    ...
    if prompt_type == "restyle":
        prompt = f"""
        Rewrite each of the following {len(data)} sentences into a new style of your choosing:
        """
        result = []
        for i, d in enumerate(data):
            if len(d["sentence"].split(" ")) < 3:
...
if __name__ == "__main__":
    # Sample a validation set of data
    data = sample_validate_set(n=50)
    # Build a GPT prompt for restyling sentences and save it
